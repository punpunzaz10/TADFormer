...
# For MTL single task performance
_C.DATA.SINGLE_TASK_DICT = CN(new_allowed=True)
...
#_C.MODEL.TADMTL.R_PER_TASK = CN(new_allowed=True)
_C.MODEL.TADMTL.SCALE_PER_TASK = CN(new_allowed=True)
_C.MODEL.TADMTL.SHARED_MODE = 'matrix'  # 'matrix', 'addition', lora_only
_C.MODEL.TADMTL.QKV_ENABLED = True
_C.MODEL.TADMTL.PROJ_ENABLED = True
_C.MODEL.TADMTL.FC1_ENABLED = True
_C.MODEL.TADMTL.FC2_ENABLED = True
_C.MODEL.TADMTL.DOWNSAMPLER_ENABLED = False

_C.MODEL.TADMTL.DTF = CN(new_allowed=True)
_C.MODEL.TADMTL.DTF.ENABLED = False
...
def update_config(config, args):
    ...
    if config.MODEL.TADMTL.ENABLED:
        ...
        elif len(config.MODEL.TADMTL.R) == 1:
            config.MODEL.TADMTL.R = config.MODEL.TADMTL.R * \
                len(config.MODEL.SWIN.DEPTHS)
        else:
            assert len(config.MODEL.TADMTL.R) == len(
                config.MODEL.SWIN.DEPTHS), "TADMTL ranks length should be the same as the number of layers"
        if not isinstance(config.MODEL.TADMTL.SHARED_SCALE, list):
            config.MODEL.TADMTL.SHARED_SCALE = [
                config.MODEL.TADMTL.SHARED_SCALE] * len(config.MODEL.SWIN.DEPTHS)
        elif len(config.MODEL.TADMTL.SHARED_SCALE) == 1:
            config.MODEL.TADMTL.SHARED_SCALE = config.MODEL.TADMTL.SHARED_SCALE * \
                len(config.MODEL.SWIN.DEPTHS)
        else:
            assert len(config.MODEL.TADMTL.SHARED_SCALE) == len(
                config.MODEL.SWIN.DEPTHS), "TADMTL shared scale length should be the same as the number of layers"
        if not isinstance(config.MODEL.TADMTL.TASK_SCALE, list):
            config.MODEL.TADMTL.TASK_SCALE = [
                config.MODEL.TADMTL.TASK_SCALE] * len(config.MODEL.SWIN.DEPTHS)
        elif len(config.MODEL.TADMTL.TASK_SCALE) == 1:
            config.MODEL.TADMTL.TASK_SCALE = config.MODEL.TADMTL.TASK_SCALE * \
                len(config.MODEL.SWIN.DEPTHS)
        else:
            assert len(config.MODEL.TADMTL.TASK_SCALE) == len(
                config.MODEL.SWIN.DEPTHS), "TADMTL task scale length should be the same as the number of layers"
        if not isinstance(config.MODEL.TADMTL.DROPOUT, list):
            config.MODEL.TADMTL.DROPOUT = [
                config.MODEL.TADMTL.DROPOUT] * len(config.MODEL.SWIN.DEPTHS)
        elif len(config.MODEL.TADMTL.DROPOUT) == 1:
            config.MODEL.TADMTL.DROPOUT = config.MODEL.TADMTL.DROPOUT * \
                len(config.MODEL.SWIN.DEPTHS)
        else:
        ...
        if len(config.MODEL.TADMTL.SCALE_PER_TASK) == 0:
            for task in config.TASKS:
                config.MODEL.TADMTL.SCALE_PER_TASK[task] = config.MODEL.TADMTL.SHARED_SCALE[:]
        else:
            for task in config.TASKS:
                if not isinstance(config.MODEL.TADMTL.SCALE_PER_TASK[task], list):
                    config.MODEL.TADMTL.SCALE_PER_TASK[task] = [
                        config.MODEL.TADMTL.SCALE_PER_TASK[task]] * len(config.MODEL.SWIN.DEPTHS)
                elif len(config.MODEL.TADMTL.SCALE_PER_TASK[task]) == 1:
                    config.MODEL.TADMTL.SCALE_PER_TASK[task] = config.MODEL.TADMTL.SCALE_PER_TASK[task] * \
                        len(config.MODEL.SWIN.DEPTHS)
                else:
